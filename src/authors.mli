(** CS 3110 Fall 2021 Assignment A2 Adventure

    @author Emory Walsh elw223, Charlotte Verity cev32, Varsha Iyer vvi2 *)

val hours_worked_ms1 : int
(** [hours_worked] is the number of hours you worked on this assignment
    during the ms1 sprint. *)

val hours_worked_ms2 : int
(** [hours_worked_ms2] is the number of hours you worked on this
    assignment during the ms2 sprint. *)

val hours_worked_ms3 : int
(** [hours_worked_ms3] is the number of hours you worked on this
    assignment during the ms3 sprint. *)
