...
let rec num_wrong_guesses state guessedLst = 
  ...
  | [] -> 0
  | h::t -> if List.mem h state.word  
    then num_wrong_guesses state t 
...
let rec wrong_list state guessedLst = 
  ...
  | [] -> []
  | h::t -> if List.mem h state.word  
    then wrong_list state t 
    else h::wrong_list state t

let rec update_board letter state=
    let () = print_endline "\n\n\n\n\n\n\n\n" in
    List.map2 (fun x y -> if x=letter then letter else y) state.word state.board

let rec right_list state guessedLst = 
  match guessedLst with
  | [] -> []
  | h::t -> if List.mem h state.word 
    then h::wrong_list state t 
    else wrong_list state t

let word_length t : int = 
    List.length t.word
...
let repr_board_state t = 
  ...
  let message = "You have "^(string_of_int (8-(num_wrong_guesses t t.guessedList)))^" guesses left." in
...
let guess_letter letter t = 
  if List.mem letter t.guessedList
  then Illegal "\n\n\nYou've already guessed this letter. Please retry.\n"
  else if not (valid_letter letter) 
  then Illegal "\n\n\nYou must enter a character A..Z. \n"
  else if valid_letter letter 
  then let new_guessed = letter::t.guessedList in
  let new_board = update_board letter t in 
  Legal {guessedList = new_guessed; board = new_board; word = t.word}
else Illegal "not valid"

let has_won t = if List.mem '_' t.board then false else true

let has_lost t = if (num_wrong_guesses t t.guessedList)=8 then true else false
